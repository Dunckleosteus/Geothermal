...
begin
	...
	T = Array{Float64, 3}(undef, nx, ny, nt); fill!(T, (T_inj + T_res) / 2)
	p = Array{Float64, 3}(undef, nx, ny, nt); fill!(p, 0.0) # Pressure
	
	# Initial conditions
	T[15:20, 15:20, 1:10] .= T_inj
	...
	for t in 1:(nt - 1)
	    for i in 2:(nx - 1)
	        for j in 2:(ny - 1)
	            ...
	            dvc_dt = -(uc[i,j,t]*dvc_dx+vc[i, j, t]*dvc_dy)+ν*(d2vc_dx2 + d2vc_dy2) +g*β*(T[i,j,t]-(T_inj+T_res)/2)
